...
Review the partially reconstructed HTTP requests and responses. Answer the questions in order to figure out what happened during the breach on your company's web server.

- An [HTTP Reference Sheet](./HTTP_Reference.md) has been provided. You are encouraged to use it when working on this activity. 

These partially reconstructed HTTP requests and responses were, at some point, exchanged with the suspicious IP address. A lot of requests and responses are missing, so we can only partially understand the attack.
